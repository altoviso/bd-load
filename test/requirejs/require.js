  ...
  var 
    ...
    getModuleInfo= function(mid, referenceModule, packages, modules, baseUrl, pageUrl, packageMapProg, pathsMapProg, pathTransforms, alwaysCreate) {
      ...
      if (/(^\/)|(\:)|(\.[^\/]+$)/.test(mid)) {
        ...
        result= modules.pqn;
        if (result) {
          return alwaysCreate ? makeModuleInfo(rsult.pid, result.mid, result.pqn, result.pack, result.path, result.url) : modules.pqn;
        }
      }
    ...
    var
      ...
      injectPlugin= function(
        ...
        if (cache[pqn]) {
          ...
          plugin.load && plugin.load(module.req, module.mid, onload);
        }
      },
