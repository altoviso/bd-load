(function(
  ...
  var
    ...
    config= function(config, booting) {
      ...
      !booting && config.deps && req(config.deps, config.cb);
    };
  ...
  var
    ...
    getModule= function(mid, referenceModule, doNotCreate) {
      ...
      if (!result) {
        result= {
          pid:targetPid, 
          mid:mainModule ? "" : targetMid,
          cqn:cqn,
          pack:targetPid && packages[targetPid],
          path:(targetPid ? targetPid : "") + (mainModule ? "/"+targetMid : (targetPid ? "/" : "") + targetMid)};
        if (!doNotCreate) {
          modules[cqn]= result;
        }
      }
      if (pluginTargetMid) {
        ...
        // TODO: this should be done by the plugin, not here
        var 
          plugin= result,
