(function(
  ...
  var
    ...
    getModule= function(mid, referenceModule, doNotCreate) {
      ...
      if (pluginTargetMid) {
        // result in actually the plugin; resolve the plugin target module with respect to referenceModule
        var 
          plugin= result,
      ...
      nameToUrl= function(
        ...
        if (resolve) {
          ...
          if (mapItem) {
            ...
            url= pack.location + "/" + (pack.lib ? pack.lib + "/" : "") + targetMid.substring(mapItem[3] + 1);
          } else {
            url= targetMid;
          }
        } else {
          pack= module.pack,
          url= (pack ? pack.location + "/" + (pack.lib ? pack.lib + "/" : ""): "" ) + name;
        }
        ...
        url= (pack && mapUrl(url, pack.urlMap)) || mapUrl(url, urlMap) || url;
        return url;
      },
